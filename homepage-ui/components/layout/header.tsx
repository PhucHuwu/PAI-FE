...
import svgPaths from '@/public/import/svg-8d5vo7up8h'
import userImg from '@/public/assets/UserIcon.svg';
import logo from '@/public/assets/Logo1.svg';
import { Menu, X, ChevronDown } from "lucide-react";
import svgThuTuc from "@/public/import/svg-5f8ts2r75e";
import svgHoTro from "@/public/import/svg-1olhhx3kod";
import svgTinTuc from "@/public/import/svg-0kyd8xp00s";
import svgTaiChinh from "@/public/import/svg-azm0jqzmsu";
...
const NAV_ITEMS: NavItemConfig[] = [
  ...
  },
  {
    ...
    dropdownWidth: 292,
    dropdown: [
      { type: "small", text: "Chính sách & Ưu đãi" },
      { type: "small", text: "Chi phí & Dịch vụ phổ biến" },
      { type: "small", text: "Quỹ & Nguồn hỗ trợ" },
    ],
  },
];
...
function SmallDropdownRow({ item, isFirst, isLast }: {
  ...
  return (
    <a
      href="#"
      className={`flex items-center gap-3 px-5 hover:bg-gray-50 transition-colors cursor-pointer border-b border-gray-100 last:border-0 bg-white ${
    ...
    >
      ...
      <p className="text-[16px] text-black">{item.text}</p>
    </a>
  );
}
...
function DropdownPanel({ items, width }: { items: DropdownItem[]; width: number }) {
  const isSmall = items[0]?.type === "small";
  return (
    <div
      className={`absolute top-full left-0 z-50 overflow-hidden ${
        isSmall
          ? "rounded-[10px] shadow-[0px_4px_12px_rgba(0,0,0,0.15)]"
          : "shadow-[0px_6px_20px_rgba(0,0,0,0.18)]"
      }`}
